...
export function TerminalAnimation() {
    ...
    const cursorVariants = {
        ...
        toInspect: isMobile ? { left: "50%", top: "35%" } : { left: "85%", top: "35%" },
        toLeft: isMobile ? { left: "30%", top: "50%" } : { left: "22%", top: "40%" },
        toRight: isMobile ? { left: "55%", top: "50%" } : { left: "48%", top: "40%" },
        toInner: isMobile ? { left: "55%", top: "40%" } : { left: "48%", top: "30%" },
    };
    ...
    return (
        <div className="relative w-full h-full bg-[#050705] overflow-hidden rounded-sm font-mono flex flex-col">
            {/* Top Bar */}
            ...
            <div className="relative h-[calc(100%-25px)] flex">
                ...
                {/* Extension Sidebar - Hidden on mobile, replaced by floating UI */}
                {!isMobile ? (
                    <div className="w-[32%] border-l border-white/20 bg-[#0A0D0A] h-full flex flex-col p-4 relative z-20 shadow-2xl overflow-hidden">
                    ...
                    <div className="absolute inset-x-0 bottom-0 p-4 z-30 pointer-events-none space-y-3">
                        <AnimatePresence>
                            {step >= 5 && step < 10 && (
                                <motion.div
                                    ...
                                    exit={{ y: 20, opacity: 0 }}
                                    className="bg-black/90 border border-white/20 p-3 rounded-sm shadow-2xl space-y-3"
                                >
                                    <div className="flex justify-between items-center">
                                        <div className="flex items-center gap-2 text-white font-bold text-[8px] uppercase tracking-wider">
                                            <Code2 size={10} className="text-primary" /> Tailwind_Source
                                        </div>
                                        <div className={`h-1.5 w-1.5 rounded-full animate-pulse ${step >= 8 ? 'bg-red-500' : 'bg-primary'}`} />
                                    </div>
                                    <code className="text-[8px] text-white/90 block font-mono">
                                        class="<span className="text-primary">
                                            {step >= 7 ? "w-full" : "w-10"}
                                            {step >= 8 ? " border-red-500" : " border-green-500"}
                                        </span>"
                                    </code>
                                </motion.div>
                            )}
